...
import psycopg2
cont =psycopg2.connect(host='localhost',
                            user='postgres',
...
icon = Image.open("Images\phonepe.jpg")
st.set_page_config(page_title="Phonepe Pulse| by Aarif",
page_icon= icon,layout='wide',initial_sidebar_state="expanded",menu_items={'About': """# This app is created by *Aarif!*"""},)
...
if selected == "Analysis":
        ...
        if option == 'All India(aggregated)':
            ...
            with tab111:
                ...
                with tab11:
                        st.tabs(['Geo Visualization of India'])
            ...
            with tab3:
                col1,col2,col3 = st.columns(3)
                with col1:
                    query = 'SELECT distinct(state) FROM map_transaction'
                    df = pd.read_sql(query, cont)
                    unique_states = df['state'].unique()
                    state_trans = st.selectbox('**Select state**',unique_states)
            ...
            with tab4:
                ...
                with col1:
                    ...
                    query1 = 'SELECT distinct(state) FROM map_user'
                    df1 = pd.read_sql(query1, cont)
                    unique_state_user = df1['state']
                    state_user = st.selectbox('**Select state**',unique_state_user,key='state_user')
        ...
        # ==============================================          /     Top categories       /             =========================================== #
            ...
            with tab5:
                ...
                with col1:
                    ...
                    query2 = 'SELECT distinct(state) FROM top_transaction'
                    df2 = pd.read_sql(query2, cont)
                    unique_top_trans_state = df2['state']
                    top_trans_state = st.selectbox('**Select state**',unique_top_trans_state)
                with col2:
            ...
            with tab6:
                ...
                with col1:
                    ...
                    query3 = 'SELECT distinct(state) FROM top_user'
                    df3 = pd.read_sql(query3, cont)
                    unique_top_user_state= df3['state']
                    top_user_state = st.selectbox('**Select state**',unique_top_user_state,key='top_user_state')
